...
export default {
	...
	getSightMessageById({ commit, state }, payload) {
		return new Promise((resolve, reject) => {
			...
			let additionMassage = activeDec[0][state.languageMessages.language[state.languageMessages.activeindex]]
	...
	getImgById({ commit, state }, payload) {
		...
		return new Promise((resolve, reject) => {
			...
			if (acitveItem.length) {
				let newarr = acitveItem[0].imgUrl.map(img => ({ backgroundImage: 'url(' + img + ')' }))
				resolve(newarr)
			} else {
				reject();
			}
	...
	getIconLanByType({ commit, state }, payload) {
		return new Promise((resolve, reject) => {
			if (IconlayerLangusge[payload]) {
				resolve(IconlayerLangusge[payload][state.languageMessages.language[state.languageMessages.activeindex]])
